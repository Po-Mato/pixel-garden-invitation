CREATE TABLE invitation_gallery (
  ...
  draft_json TEXT NOT NULL CHECK (length(draft_json) BETWEEN 2 AND 32768 AND json_valid(draft_json)),
  draft_revision INTEGER NOT NULL DEFAULT 1 CHECK (draft_revision >= 1),
  published_json TEXT CHECK (
    published_json IS NULL
    OR (length(published_json) BETWEEN 2 AND 32768 AND json_valid(published_json))
  ),
...
CREATE TABLE invitation_gallery_versions (
  id TEXT PRIMARY KEY,
...
CREATE INDEX idx_invitation_gallery_versions_recent
  ON invitation_gallery_versions(invitation_id, created_at DESC, id DESC);

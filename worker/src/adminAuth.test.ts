...
function createDb() {
  ...
  return { db: { prepare } as unknown as D1Database, attempts, bindCalls };
}
...
describe("attemptAdminLogin", () => {
  ...
  it("starts a new window only after ten minutes have elapsed", async () => {
    const { db } = createDb();
    ...
    await expect(attemptAdminLogin(db, loginInput({ now: start.now + 10 * 60 * 1_000 + 1 })))

...
`guest-11`은 검은 볼륨 가르마 헤어, 딥 그린 블레이저, 아이보리 오픈칼라 셔츠, 크림 팬츠 프리셋으로 같은 구조와 검증 기준을 사용한다.

- 턴어라운드: `character-assets/reference/guest-3d-master-sources/v1/guest-11/guest-11-turnaround-concept.png`
- 3D 마스터: `character-assets/reference/guest-3d-master-sources/v1/guest-11/blender/guest-11-master.blend`
- 앱 적용 후보와 감사 결과: `character-assets/reference/guest-3d-master-sources/v1/guest-11/pilot`
- Live2D 계획: `character-assets/live2d/guest-11/model-plan.json`

`guest-12`는 검은 가르마 헤어, 네이비 모던 한복 배자, 아이보리 저고리, 회청색 바지, 은색 노리개 프리셋으로 같은 구조와 검증 기준을 사용한다.

- 턴어라운드: `character-assets/reference/guest-3d-master-sources/v1/guest-12/guest-12-turnaround-concept.png`
- 3D 마스터: `character-assets/reference/guest-3d-master-sources/v1/guest-12/blender/guest-12-master.blend`
- 앱 적용 후보와 감사 결과: `character-assets/reference/guest-3d-master-sources/v1/guest-12/pilot`
- Live2D 계획: `character-assets/live2d/guest-12/model-plan.json`
...
blender --background --python scripts/blender/build_guest_3d_master.py -- --guest guest-11 --output-root character-assets/reference/guest-3d-master-sources/v1/guest-11/blender
node scripts/render-guest-3d-pose-guides.mjs --root character-assets/reference/guest-3d-master-sources/v1/guest-11/blender
node scripts/build-guest-3d-sprite-pilot.mjs --guest guest-11 --preset masculine-green-blazer-cream-pants

blender --background --python scripts/blender/build_guest_3d_master.py -- --guest guest-12 --output-root character-assets/reference/guest-3d-master-sources/v1/guest-12/blender
node scripts/render-guest-3d-pose-guides.mjs --root character-assets/reference/guest-3d-master-sources/v1/guest-12/blender
node scripts/build-guest-3d-sprite-pilot.mjs --guest guest-12 --preset masculine-blue-modern-hanbok
```

## 오픈소스 2D 퍼펫 적용 범위

현재 구현은 상용 Cubism 런타임과 편집기 산출물을 사용하지 않는다. MIT 라이선스의 PixiJS 8을 렌더러로 사용하며, 분리된 머리·몸·눈 감은 표정 레이어를 뼈대 컨테이너에 연결한 2D 퍼펫이다. 보행 스프라이트를 대체하지 않고 입장 화면과 소개 화면에만 제한적으로 활성화한다.

- 입장 화면: 눈 깜빡임, 호흡, 작은 고개 움직임
- 신랑·신부 소개: 눈 깜빡임, 호흡, 작은 고개 움직임
- 화면 밖, 저사양 모드, 데이터 절약, 모션 감소 설정: 정적 WebP 대체 이미지

현재 납품물:

- 원본: `character-assets/reference/couple-puppet-sources/v1/{bride|groom}`
- 분리 레이어와 리그: `character-assets/source/couple-puppets/{bride|groom}`
- 앱 자산: `client/public/characters/puppets/{bride|groom}`
- 리그 정보: 캐릭터별 `rig.json`
- 재생성: `pnpm characters:build-couple-puppets`
- 런타임 라이선스: PixiJS, MIT

한 장을 통째로 흔드는 방식은 사용하지 않는다. 몸 뼈대와 머리 뼈대를 독립적으로 움직이고, 열린 눈과 감은 눈을 별도 표정 레이어로 전환한다. 다음 확장 시에는 앞머리, 뒷머리, 팔, 부케와 의상을 추가 분리한다. 기존 `character-assets/live2d` 경로는 하객별 분해 계획의 역사적 이름으로 유지하며 실제 런타임 형식을 의미하지 않는다.

## 확대 순서

1. `guest-01`을 실제 화면에서 확인한다.
2. 같은 규격으로 `guest-02`부터 `guest-12`까지 3D 마스터와 보행 시트를 만든다.
3. 신랑·신부 3D 마스터를 먼저 정리한 뒤 오픈소스 2D 퍼펫 리깅 원본을 제작한다.
4. 신랑·신부 2D 퍼펫을 입장 화면과 소개 화면에 제한적으로 적용한다.

...
- [x] **Step 1: 다음 프롬프트로 초광폭 차량 레퍼런스와 픽셀 배경을 생성한다**
...
- [x] **Step 4: 긴 카메라 이동과 양쪽 포털을 검증한다**
...
- [x] **Step 1: 다음 프롬프트로 레퍼런스·픽셀 배경·꽃 아치를 생성한다**
...
- [x] **Step 2: 다음 데이터로 `venueExteriorZone`을 교체한다**
...
- [x] **Step 3: 중앙 통로와 포털 도착 연출을 검증한다**
...
- [x] **Step 1: 다음 프롬프트로 화장실 자산을 생성한다**
...
- [x] **Step 2: 다음 데이터로 `restroomZone`을 교체한다**
...
- [x] **Step 1: 실제 manifest와 월드 크기 대응 테스트를 추가한다**
...
- [x] **Step 2: 루트 test와 build 앞에 `maps:audit`를 연결한다**
...
- [x] **Step 1: 사용 가능한 포트를 확인하고 로컬 서버를 시작한다**

Run: `pnpm dev -- --host 127.0.0.1`

Expected: 기존 포트가 사용 중이면 다른 포트로 실행되고 URL이 출력됨.

- [x] **Step 2: 모바일 네 크기와 데스크톱에서 전체 여정을 자동화한다**
...
배포 승인 후 `main`의 `8acc523`까지 푸시했고 GitHub Pages 실행 `29452169502`가 성공했다. 실제 서비스에서 10개 구역의 배경·전경 자산 20개가 모두 HTTP 200으로 응답했으며, `390x844`와 `1440x1000`에서 포털 보행·도착·페이드 전환을 통해 전체 구역을 순회했다. 각 구역의 배경 로드, 전경 디코딩, 미니맵 동기화와 브라우저 콘솔 무오류를 확인했고 기존 미추적 캐릭터 디렉터리는 푸시 대상에서 제외했다.

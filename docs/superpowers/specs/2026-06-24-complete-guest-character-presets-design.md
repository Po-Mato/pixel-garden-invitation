...
상태: 사용자 방향 전환 확정, 구현 후 기준 이미지 4명 복구 방식으로 정정
...
초기 프리셋은 4개로 제한한다. 수량보다 품질을 우선하며, 확정된 기준 이미지의 네 캐릭터를 그대로 런타임 프리셋의 기준으로 사용한다.
...
완성 프리셋은 기준 이미지 crop을 직접 사용한다. 기준 이미지 crop은 배경을 투명화한 뒤 `96x144` 스프라이트로 정렬한다. 단순 도형, 색면, 원형 얼굴을 새로 그리는 procedural 저작 방식은 금지한다.
...
- 컨택트 시트에서 4개 하객이 모두 기준 이미지의 “예쁘고 멋진 하객” 톤에 맞게 보인다.

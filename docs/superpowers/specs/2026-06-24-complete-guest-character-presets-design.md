...
완성 프리셋은 기준 이미지 crop을 직접 사용한다. 기준 이미지 crop은 배경을 투명화한 뒤 `96x144` 스프라이트로 정렬한다. walk 시트는 `down`, `left`, `right`, `up` 네 방향 행을 모두 제공해야 하며, 네 행을 같은 정면 프레임으로 복제하지 않는다. `down`은 승인 정면 crop을 유지하고, `left`/`right`는 프로필 실루엣, `up`은 얼굴이 보이지 않는 뒷모습으로 파생한다. 단순 도형, 색면, 원형 얼굴을 새로 그리는 procedural 저작 방식은 금지한다.

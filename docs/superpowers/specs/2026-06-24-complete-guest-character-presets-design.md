...
각 프리셋은 얼굴, 헤어, 의상, 액세서리가 이미 함께 설계된 기준 이미지 crop에서 나온다. 얼굴 중앙 보호를 위해 헤어를 억지로 제한하는 방식이 아니라, 승인된 완성 캐릭터 자체가 사람다운 얼굴과 균형 잡힌 실루엣을 갖도록 고정한다.
...
완성 프리셋은 기준 이미지 crop에서 방향별 원본 PNG를 먼저 만든 뒤 사용한다. `character-assets/reference/guest-directions/{preset-id}/{direction}.png`가 방향별 master이며, `author-guest-preset-sources`는 이 파일들을 배경 투명화한 뒤 `96x144` 스프라이트로 픽셀화한다. walk 시트는 `down`, `left`, `right`, `up` 네 방향 행을 모두 제공해야 하며, 네 행을 같은 정면 프레임으로 복제하지 않는다. `down`은 승인 정면 방향 원본을 유지하고, `left`/`right`는 프로필 실루엣, `up`은 얼굴이 보이지 않는 뒷모습이어야 한다. 단순 도형, 색면, 원형 얼굴을 새로 그리는 procedural 저작 방식은 금지한다.
...
- 승인된 기준 이미지 crop에서 완성 프리셋 소스 PNG를 생성하는 스크립트 추가
...
- 컨택트 시트에서 12개 하객이 모두 기준 이미지의 “예쁘고 멋진 하객” 톤에 맞게 보인다.

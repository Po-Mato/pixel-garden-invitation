...
def set_walk_animation(root, left_leg, right_leg, left_arm, right_arm):
    poses = (
    ...
    if root.animation_data and root.animation_data.action:
        root.animation_data.action.name = "Guest01_Walk"
...
def main():
    ...
    root, left_leg, right_leg, left_arm, right_arm = build_character()
    set_walk_animation(root, left_leg, right_leg, left_arm, right_arm)
    render_frames(root, output_root)

    root.rotation_euler.z = 0
    bpy.context.scene.frame_set(7)
    blend_path = output_root / "guest-01-master.blend"
    bpy.ops.wm.save_as_mainfile(filepath=str(blend_path))
    for obj in bpy.context.scene.objects:
        obj.select_set(obj.type not in {"CAMERA", "LIGHT"})
    bpy.context.view_layer.objects.active = root
    bpy.ops.export_scene.gltf(
        filepath=str(output_root / "guest-01-master.glb"),
        export_format="GLB",
        use_selection=True,

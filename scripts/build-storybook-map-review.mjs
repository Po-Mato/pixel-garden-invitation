...
const characters=await Promise.all(catalog.characters.map(async c=>{
 const url=new URL(`character-assets/generated/storybook-runtime-staging-v1/${c.presetId}/${c.presetId}__walk-runtime.png`,root);
...
const studyInputs=await Promise.all(['scripts/build-storybook-map-review.mjs','scripts/templates/storybook-map-review.js'].map(async file=>({file,sha256:createHash('sha256').update(await readFile(new URL(file,root))).digest('hex')})));
const data={zones,characters,thresholds:contract.thresholds,inputs:[...await guest216MapInputs(root),...studyInputs],cssSha256:createHash('sha256').update(css).digest('hex')};
...
const script=profiles+await readFile(new URL('scripts/templates/storybook-map-review.js',root),'utf8');
await writeFile(new URL('character-assets/rigs/storybook-expansion-v1/map-browser-review.html',root),`<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>새 페인팅 맵 대비 대조</title><style>body{margin:12px;font:14px system-ui;background:#eee8df;color:#26334a}h1{font-size:19px}select,button{font:inherit;padding:8px;margin:3px}#grid{display:grid;grid-template-columns:repeat(4,84px);gap:7px}figure{margin:0}canvas{display:block;width:84px;height:104px}figcaption{font-size:11px;line-height:16px}#status{white-space:pre-wrap}</style><h1>새 페인팅 · 맵 대비 대조</h1><p>실제 맵 위치·전경 + 브라우저 색상/외곽 필터.<br>별도 검수 도구이며 실제 게임 UI 검증을 대체하지 않음.</p><select id="zone" aria-label="맵"></select><select id="direction" aria-label="방향"><option value="0">정면</option><option value="1">왼쪽</option><option value="2">오른쪽</option><option value="3">후면</option></select><select id="frame" aria-label="프레임"><option value="1">중립 2</option><option value="0">보행 1</option><option value="2">보행 3</option><option value="3">중립 4</option></select><button id="audit">전체 대비 검사</button><p id="status">로딩 중</p><div id="grid"></div><script>const DATA=${JSON.stringify(data)};${script}</script></html>`);
